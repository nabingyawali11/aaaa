...
- **birthday/SomethingToTellYou.jsx** — Dark slate + pink "confession" realm (Hero → The Confession → Three Promises → Final CTA) with floating hearts; routed at `/something-to-tell-you` and `/test2`
...
- **vite.config.js** — Dev proxy for `/api/cloudinary-search` → Cloudinary; credentials read from env via `loadEnv()` (no hardcoded secrets). Also mounts an `api-log-attempt-dev` middleware plugin exposing `/api/log-attempt` in dev (loads `DATABASE_URL` from env)
